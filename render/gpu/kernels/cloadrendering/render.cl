...
__kernel void renderClouds(
    ...
    float phase      = (hg < 4.0f) ? hg : 4.0f; /* min() inlined to avoid _cl_* call */
...
// Fuse the two per-step gathers of godRays (cloud transmittance .w and the
// sceneDepth isSky test) into a single precomputed per-pixel source.
// src[i] = cloudTransmittance[i] * isSky[i]. Runs once per frame on the
// post-blur cloud buffer; godRays then gathers ONE 4-byte float per step
// instead of a 16-byte float4 + 4-byte float (2 scattered loads -> 1).
__kernel void buildGodRaySource(
    __global const float4 *cloudBuffer,
    __global const float  *sceneDepth,
    int    screenWidth,
    int    screenHeight,
    __global float        *out
) {
    ...
    if (x >= screenWidth || y >= screenHeight) return;
    int idx = y * screenWidth + x;
    float isSky = (sceneDepth[idx] >= 1e29f) ? 1.0f : 0.0f;
    out[idx] = cloudBuffer[idx].w * isSky;
}

// Screen-space radial march from each pixel toward the sun.
// Reads the fused god-ray source (transmittance*isSky) as an occlusion mask; output is additive RGB.
__kernel void godRays(
    __global const float  *godRaySrc,
    int    screenWidth,
    int    screenHeight,
    float2 sunScreenPos,
    float3 godRayColor,
    ...
    for (int i = 0; i < GOD_RAY_STEPS; i++) {
        ...
        illumination += godRaySrc[si] * accumDecay * w;
        accumDecay   *= decay;
    }
