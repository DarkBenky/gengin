// =============================================================================
//  planeSurfacesExample.c  –  Self-contained aerodynamics library for flight simulation
//
...
static inline float3 f3_scale(float3 a, float s) {
	return (float3){a.x * s, a.y * s, a.z * s};
}
static inline float3 f3_neg(float3 a) {
...
static inline float f3_len(float3 a) {
	return sqrtf(f3_dot(a, a));
}
static inline float f3_len2(float3 a) {
	return f3_dot(a, a);
}
static inline float3 f3_norm(float3 a) {
	float l = f3_len(a);
	return l > 1e-8f ? f3_scale(a, 1.0f / l) : (float3){0, 0, 0};
}
...
// --- quaternion helpers ---
static inline float4 quat_identity(void) {
...
static void aerofoilCoeffs(const AerofoilProfile *p, float alpha,
	...
	float Cl = Cl_lin * (1.0f - t_pos) * (1.0f - t_neg) + lerpf(Cl_lin, p->clMax, t_pos) * t_pos * (1.0f - t_neg) + lerpf(Cl_lin, p->clMin, t_neg) * t_neg * (1.0f - t_pos);
	...
	float Cd = p->cd0 + p->cd2 * Cl * Cl + stall_full * 0.15f * sinf(alpha) * sinf(alpha);
...
static inline FlapConfig flapConfigDefaults(void) {
	return (FlapConfig){
		{0, 1, 0}, {1, 0, 0}, 0.5f, 0.698f, 4.5f, 0.82f, 0.012f, 0.08f, 1.05f, 0.15f, 0.5f, 1.0f};
}
...
float3 calculateFlapForce(const FlapConfig *cfg, FlapState *state) {
	...
	float Cd = cfg->cd0 + cfg->cdDeflectionScale * fabsf(sinf(defl)) + Cl * Cl / (PI * cfg->aspectRatio * cfg->oswaldEfficiency);
...
typedef enum { WING_SIDE_LEFT = -1,
			   WING_SIDE_RIGHT = 1,
			   WING_SIDE_CENTER = 0 } WingSide;
...
static inline WingState wingStateDefaults(void) {
	...
	memset(&s, 0, sizeof(s));
	s.airDensity = 1.225f;
	s.speedOfSound = 340.0f;
	s.groundHeight = 1e5f;
	for (int i = 0; i < WING_MAX_FLAPS; i++)
		s.flapStates[i] = flapStateDefaults();
	return s;
}
...
static inline FuselageState fuselageStateDefaults(void) {
	...
	s.airDensity = 1.225f;
	s.speedOfSound = 340.0f;
	s.bodyForwardAxis = f3(0, 0, -1);
	s.bodyUpAxis = f3(0, 1, 0);
	return s;
}
...
static inline MissileState missileStateDefaults(void) {
	...
	s.fuselageState = fuselageStateDefaults();
	for (int i = 0; i < MISSILE_MAX_FINS; i++)
		s.finStates[i] = wingStateDefaults();
	return s;
}
...
#define BODY_TO_WORLD_M(v) quat_rotate(state->orientation, v)
	...
	for (int fi = 0; fi < cfg->finCount; fi++) {
		...
		float deflection = fin->pitchAxis * state->pitchCommand + fin->yawAxis * state->yawCommand + fin->rollAxis * state->rollCommand;
