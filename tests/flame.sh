#!/usr/bin/env bash
# Perf-sample a test binary and render a FlameGraph SVG.
#
#   tests/flame.sh build/tests/AOBench_flame build/tests/flame/AOBench.svg
#
...
# missing graph must never fail the caller: test results stay authoritative.
set -u

bin=${1:-}
svg=${2:-}
if [[ -z $bin || -z $svg ]]; then
	echo "usage: $0 <test-binary> <out.svg>" >&2
	exit 1
fi
if [[ ! -x $bin ]]; then
...
hz=999
rm -f "$data"
if ! perf record -F "$hz" -g --call-graph fp -o "$data" -- "$bin" >/dev/null 2>"$data.err"; then
	...
	rm -f "$data"
	if ! sudo perf record -F "$hz" -g --call-graph fp -o "$data" -- "$bin" >/dev/null 2>"$data.err"; then
